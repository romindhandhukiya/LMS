...
const registerUser = async (req, res) => {
  ...
  try {
    ...
    const userExist = await employees.findOne({
    ...
    if (userExist) {
      ...
      return res
        .status(422)
        .json({ error: "password and confirm password doesnot match" });
...
const getRole = async (req, res) => {
  try{
    ...
    const decoded = await jwt.verify(token, process.env.SECRET_KEY);
        ...
        res.json({"message" : "Roles verified", role});
  }
  catch (error) {
    console.log(error);
  }
...
const verifyPassOtp = async function (req, res) {
  try {
    ...
    const isMatch = await otpModel.findOne({
    ...
    const isUsed = await otpModel.findOne({
      $and: [{ email: email, otp: otp, otpUsed: 0 }],
    });
    if (!isUsed) {
...
const resetPassword = async function (req, res) {
  try {
    ...
    const isUsed = await otpModel.findOne({
      $and: [{ email: email, otp: otp, otpUsed: 1 }],
    });
    if (!isUsed) {
      res.json({ error: "Invalid Otp" });
    }
